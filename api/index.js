...
router.post('/send-code-phone', async (req, res) => {
    ...
    const data = {
        ...
        message_text: 'Your BlackTribe Fashion confirmation code is < 1234 >. It expires in 30 minutes',
        pin_type: 'NUMERIC'
    };
...
router.post('/order-placed-sms', async (req, res) => {
    const message = `Thanks for your order (ID ${req.body.order_id})! Payment being confirmed. Order processing soon. SMS updates will be sent. View your order at blacktribefashion.com.`;
    const data = {
        to: req.body.phone_number,
        from: "HairbyTimaB",
        sms: message,
...
router.post('/order-confirmed-sms', async (req, res) => {
    const message = `Yay! Your order (ID ${req.body.order_id}) has been confirmed! We're processing it for delivery. You'll receive SMS updates. View your order at blacktribefashion.com.`;
    const data = {
        to: req.body.phone_number,
        from: "HairbyTimaB",
        sms: message,
...
router.post('/order-shipped-sms', async (req, res) => {
    const message = `Exciting update! Your order (ID ${req.body.order_id}) is now shipped and will soon be delivered! Track it at blacktribefashion.com. Thank you for choosing Blacktribe Fashion!`
    const data = {
        to: req.body.phone_number,
        from: "HairbyTimaB",
        sms: message,
...
router.post('/order-delivered-sms', async (req, res) => {
    const message = `Great news! Your order (ID ${req.body.order_id}) has been delivered! Track it at blacktribefashion.com. Thank you for choosing Blacktribe Fashion, we can't wait to serve you again`
    const data = {
        to: req.body.phone_number,
        from: "HairbyTimaB",
        sms: message,
...
router.post('/send-code-email', async (req, res) => {
    ...
    let transporter = nodemailer.createTransport({
        ...
        port: 2525,
        auth: {
            user: "support@blacktribefashion.com",
            pass: "tits",
        },
    });
